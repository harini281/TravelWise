        ...
        try
        {
            var weather =
                await _weatherService
                    .GetCurrentWeatherAsync(trip.Destination);
    ...
    AssessTripRisk(int tripId)
{
    ...
    if (weather == null)
    {
        return StatusCode(
            503,
            "Weather data is unavailable. Risk assessment cannot be completed safely.");
    }

    int riskScore = 0;

    // Project deterministic rule:
    // unusually high/low temperatures increase risk.
    ...
    // Project deterministic wind rules.
    ...
    // Project deterministic weather-code rules.
    ...
    string summary = riskLevel switch
    {
        ...
        "MODERATE" =>
            "Some weather conditions may affect the trip. Travellers should remain cautious.",

        "HIGH" =>
            "Weather conditions may significantly affect planned activities. Safer alternatives should be considered.",

        "CRITICAL" =>
            "Current conditions indicate serious travel risk. High-risk activities should not proceed without review.",
